...
fechaHoraInicio    = datetime([2014  1  1  0  0  0],'TimeZone',zonaHoraria);
fechaHoraFin       = datetime([2014 12 30 0 0 0],'TimeZone',zonaHoraria);
...
while t1<fechaHoraFin
    ...
    if currentFileDate ~= nextFileDate
        %Mnext = leeMETfecha(directorio,t2); % QUITAR SI DESCOMENTO LO OTRO
        if flag == 1
            t2 = t2 - duration(24, 0 ,0);
            flag = 0;
        end
        ...
        try
            ...
            fprintf('Fallo! Fecha no existente \n') 
            fprintf('Se copia el valor del día anterior... \n') 

            t2 = t2 - duration(24, 0 ,0);
            Mnext = leeMETfecha(directorio,t2);
            t2 = t2 + 2*duration(24,0,0);
            flag = 1;
        end
